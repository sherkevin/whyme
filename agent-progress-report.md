...
## Milestone 93 - 2026-05-10 13:16 (UTC+8) - Codex

- Closed `todo-tasks.md` §18.31 after the browser review caught a real regression: item detail clicks could still bubble into the business prototype's static `openDrawer("itemDetail")`, showing the hard-coded `2025 年 AI 产品设计趋势洞察` drawer.
- Frontend fix: `bindCardClickToDrawer()` now captures all inspiration/record card clicks, blocks the prototype fallback with `preventDefault()` + `stopImmediatePropagation()`, shows a real-data loading state, then opens the drawer only after `/api/v1/cards/{id}` returns. Rows without a real `data-card-id` are blocked with a warning instead of showing mock content.
- Drawer hydration now updates the header meta from the card payload (`content_type`, real timestamp, AI summary source, model) and hides the static source/provenance section, so title, summary, tags, raw content, source, and meta all come from the database.
- Browser QA @ `http://localhost:8000/mydow/biz_v14/`: clicked `ChatGPT 调研报告总结`; drawer showed the clicked title plus `5月10日 03:22 · 真实 AI 摘要 · GLM-4-Flash`, real summary/tags/raw content, `containsStaticTitle=false`, `containsStaticMeta=false`, console errors/warnings `0`.
- Verification: `node --check static\mydow\biz_v14\bridge_v14.js`; focused tests `2 passed`; full `pytest tests\integration\api\test_prd10_frontend_binding.py -q` -> `55 passed`; `python scripts\audit_v14_buttons.py` -> `OK`.
